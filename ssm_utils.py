...
def getRotMat(rxryrz):
    return o3d.geometry.get_rotation_matrix_from_zyx(
        rxryrz[::-1]
    ).T  # 先进行x轴旋转，再y轴，再z轴；取转置表示右乘旋转矩阵
...
def getEigValVecOfSSMByPCA(alignedPointGroups):
    alignedPointVectors = np.array(
        [pg.flatten() for pg in alignedPointGroups], dtype=np.float32
    )  # shape=(sampleNum, 3*pointNum)
    ...
    sampleNum = A.shape[0]
    convMat = 1.0 / (sampleNum - 1) * A.T @ A  # 需要对协方差矩阵A.T @ A进行主成分分析
    equConvMat = 1.0 / (sampleNum - 1) * A @ A.T  # A@A.T和A.T@A的特征值相同
...
def saveEigValVec(srcRootDir, NumPC2Save):
    ...
    for toothIndex in toothIndices:
        ...
        np.save(
            os.path.join(saveDir, "meanAlignedPG_{}.npy".format(toothIndex)),
            meanAlignedPG,
        )
...
def visualizeCompactnessOfSSM(eigVal):
    ...
    plt.ylim([0.0, 1.0])
    plt.xlim([0.0, len(eigVal)])
    plt.plot(
        np.arange(0, len(eigVal) + 1),
        np.hstack([np.array([0.0]), varContriCumSum]),
        linewidth=1,
        color="r",
        linestyle="-",
        marker="o",

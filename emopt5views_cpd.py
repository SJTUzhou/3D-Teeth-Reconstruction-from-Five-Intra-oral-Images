        ...
        for phType, paramDict in ExtrParamSearchSpace.items():
            ...
            for paramName, paramSearchSpace in paramDict.items():
                ...
                for paramValue in paramSearchSpace:
                    ...
                    X_Mu_preds.append(self.X_Mu_pred[ph])
                ...
                losses = []
                for i in range(len(paramSearchSpace)):
                    self.updateCameraParams(TP_preds[i], X_Mu_preds[i], phType, self.rela_txyz, self.rela_R) # update extrinsic and intrinsic camera params
                ...
                self.updateCameraParams(TP_preds[idx_selected], X_Mu_preds[idx_selected], phType, self.rela_txyz, self.rela_R) # update extrinsic and intrinsic camera params
            print("-"*50)
        ...
        for paramName, (paramSearchSpace, phTypes) in RelativePoseParamSearchSpace.items():
            ...
            for idx, paramValue in enumerate(paramSearchSpace):
                ...
                for jdx, phType in enumerate(phTypes):
                    ph = phType.value
                    ii = idx * num_photo_relevant + jdx
                    self.updateCameraParams(TP_preds[ii], X_Mu_preds[ii], phType, self.rela_txyz) # update extrinsic and intrinsic camera params
            ...
            print("Best param guess: ", bestParamValue)
            for jdx, phType in enumerate(phTypes):
                ii = idx_selected * num_photo_relevant + jdx
                self.updateCameraParams(TP_preds[ii], X_Mu_preds[ii], phType, self.rela_txyz) # update extrinsic and intrinsic camera params
            print("-"*50)
    ...
    def searchDefaultRelativePoseParams(self):
        ...
        for paramName, SearchSpace in RelativePoseParamSearchSpace.items():
            ...
            for i,paramValue in enumerate(SearchSpace):
                self.assignValue2RelaPoseParamByName(paramName, paramValue) # 初始化牙列相对位姿参数
                self.updateCameraParams(TP_preds[i], X_Mu_preds[i], phType, self.rela_txyz) # update extrinsic and intrinsic camera params
                losses.append(self.expectation_step(phType, stage=0, verbose=True, use_percentile=False)) # use expectation loss as evaluation metric

            idx_selected = np.argmin(losses)
            bestParamValue = SearchSpace[idx_selected] # best guess from expectation loss
            self.assignValue2RelaPoseParamByName(paramName, bestParamValue, assign2DefaultValue=True) # update default values with the best guess
            print("Best param guess: ", bestParamValue)
            self.updateCameraParams(TP_preds[idx_selected], X_Mu_preds[idx_selected], phType, self.rela_txyz) # update extrinsic and intrinsic camera params
        print("-"*50)

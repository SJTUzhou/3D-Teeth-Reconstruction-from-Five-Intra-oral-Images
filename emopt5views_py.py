...
class EMOpt5Views(object):
    ...
    def __init__(self, edgeMasks, photoTypes, visMasks, Mask, Mu, SqrtEigVals, Sigma, PoseCovMats, ScaleCovMat, transVecStd, rotAngleStd) -> None:
        ...
        self.X_deformed = np.empty(self.X_Mu.shape, dtype=np.float64)
        self.X_deformed_normals = np.empty(self.X_Mu.shape, dtype=np.float64)
        self.RotMats = np.empty((self.numTooth,3,3), dtype=np.float64)
        self.X_trans = np.empty(self.X_Mu.shape, dtype=np.float64)
        self.X_trans_normals = np.empty(self.X_Mu.shape, dtype=np.float64)
    ...
    def initRelativeToothRowPose(self):
        self.rela_rxyz = np.array([0.,0.,0.],dtype=np.float64) #下牙列相对于上牙列的旋转
        self.rela_R = self.updateRelaRotMat(self.rela_rxyz)
        self.rela_txyz = np.array([0.,-5.,0.],dtype=np.float64) #下牙列相对于上牙列的位移

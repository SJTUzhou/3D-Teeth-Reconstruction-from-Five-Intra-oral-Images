...
def Dice_SSIM_loss(y_true, y_pred, SSMI_Weight=1.):
...
def get_data_filenames(parent_dir):
    image_filename_list = sorted(glob.glob(os.path.join(parent_dir, IMAGE_SUBDIR, '*.png')))
    label_filename_list = sorted(glob.glob(os.path.join(parent_dir, LABEL_SUBDIR, '*.png')))
...
def train(model, weight_ckpt=r"./'weights-teeth-boundary-model.h5", batch_size=2, epochs=50, lr=0.0005, loss_func=Dice_SSIM_loss):
    model.summary()
    ...
    ret = model.fit(x=train_dg, validation_data=valid_dg, epochs=epochs, verbose=2, callbacks=[model_checkpoint])

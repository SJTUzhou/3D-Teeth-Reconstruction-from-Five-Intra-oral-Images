...
NUM_PC = 10
SETTINGS = r"Ground-truth-Grad-99%conf-v21-PC=10" 
# r"Grad-v21-PC=10"
# r"Grad-99%conf-v21-PC=10-FixedInit"
# r"Grad-99%conf-v21-PC=10-NoPoint2PlaneLoss-NoLogLikelihood" 
# r"Grad-99%conf-v21-PC=10-NoPoint2PlaneLoss" 
# r"Grad-99%conf-v21-PC=10-NoLogLikelihood" 
# r"Grad-99%conf-v21-PC=10"
DEMO_H5_DIR = r"./dataWithPhoto/demo/{}/".format(SETTINGS)
EVAL_SSM_CSV = r"./dataWithPhoto/_temp/evaluation_SSM.csv"
EVAL_3D_RE_CSV = r"./dataWithPhoto/_temp/evaluation_3Dre_{}.csv".format(SETTINGS)
EVAL_3D_RE_STATIS_CSV = r"./dataWithPhoto/_temp/3Dre_statistics_{}.csv".format(SETTINGS)
...
def evaluate_SSM(TagID, Mu, Sigma):
    ...
    CDs = np.array(metric.computeChamferDistance(_X_Ref, _Re_X_Ref, return_list=True))
...
def evaluation_3D_reconstruction(TagID):
    ...
    CD_T_pred = np.array(metric.computeChamferDistance(_X_Ref, _TX_Pred, return_list=True))
    ...
    Metrics_array = np.hstack([np.tile(TagID,(len(indices),1)), indices[:,None], RMSD_T_pred[:,None], ASSD_T_pred[:,None], HD_T_pred[:,None], CD_T_pred[:,None], Dice_VOE_T_pred])
    df = pd.DataFrame(data=Metrics_array, columns=["tagID", "toothID", "RMSD", "ASSD", "HD", "CD", "DSC", "VOE"])
    df = df.astype({"tagID":int,"toothID":int})
    print("Finish TagID: ", TagID)
    return df
...
def analyze_metrics_per_tooth_type(metric_csv):
    ...
    for tID in TOOTH_INDICES:
        ...
        _statistics_df = _metrics[["RMSD", "ASSD", "HD", "CD", "DSC", "VOE"]].describe(percentiles=[.5])
...
def analyze_metrics(metric_csv):
    ...
    statistics_df = df[["RMSD", "ASSD", "HD", "CD", "DSC", "VOE"]].describe(percentiles=[.5])
    print(statistics_df)

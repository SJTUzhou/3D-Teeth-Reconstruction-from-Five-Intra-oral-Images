...
class EMOpt5Views(object):
    ...
    def __init__(self, edgeMasks, photoTypes, visMasks, Mask, Mu, SqrtEigVals, Sigma, PoseCovMats, ScaleCovMat, transVecStd, rotAngleStd) -> None:
        ...
        self.rowScaleXZ = np.ones((4,), dtype=np.float32) # 各向异性牙列放缩, 仅用于maximization stage1, 在stage2之前转化为scale和transVecXYZs
        ...
        self.updateAlignedPointCloudInWorldCoord() # 更新上下牙列世界坐标系中的三维预测
    ...
    ######### Update in E step ################
    ###########################################
    def updateAlignedPointCloudInWorldCoord(self):
        # 暂未考虑下牙列相对于上牙列的位姿关系
        tIdx = [i for i in range(self.numTooth)]
        self.X_deformed = self.updateDeformedPointPos(self.featureVec, tIdx)
        self.X_deformed_normals = self.computePointNormals(self.X_deformed)

        self.RotMats = self.computeTeethRotMats(self.rotAngleXYZs)
        self.X_trans = self.updateTransformedPointPos(self.X_deformed, self.scales, self.RotMats, self.transVecXYZs, tIdx)

        self.X_trans[:self.numUpperTooth] = np.hstack([self.rowScaleXZ[0],1.,self.rowScaleXZ[1]]) * self.X_trans[:self.numUpperTooth]
        self.X_trans[self.numUpperTooth:] = np.hstack([self.rowScaleXZ[2],1.,self.rowScaleXZ[3]]) * self.X_trans[self.numUpperTooth:]
    ...
    def expectation_step_5Views(self, verbose=True):
        ...
        self.updateAlignedPointCloudInWorldCoord()
    ...
    def computePixelResidualError(self, photoType, featureVec, scales, rotAngleXYZs, transVecXYZs, extrViewMat, intrProjMat,\
        rela_R, rela_txyz, rowScaleXZ=np.ones((4,),np.float32), stage=1):
        ...
        if stage == 1: # 在优化相机参数同时，优化牙列的Anistropic scales
            if photoType == PHOTO.UPPER:
                ...
                X_corre_pred = np.hstack([rowScaleXZ[2],1.,rowScaleXZ[3]]) * X_corre_pred
            else:
                ul_sp = self.ul_sp[ph]
                ks = len(np.vstack(X_trans_pred[:ul_sp]))
                idx_u = self.corre_pred_idx[ph] < ks
                idx_l = self.corre_pred_idx[ph] >= ks
                X_corre_pred[idx_u] = np.hstack([rowScaleXZ[0],1.,rowScaleXZ[1]]) * X_corre_pred[idx_u]
                X_corre_pred[idx_l] = np.hstack([rowScaleXZ[2],1.,rowScaleXZ[3]]) * X_corre_pred[idx_l]
    ...
    def residualErrorOfGlobalParams(self, params, pIdx, photoType, withAniScales=False, verbose=True):
        ...
        rowScaleXZ = np.ones((4,), np.float32)
        errorAniScales = 0.
        if withAniScales == True: # 考虑牙列各向异性放缩
            rowScaleXZ = params[pIdx["rowScaleXZ"]:pIdx["rowScaleXZ"]+4] 
            # errorAniScales = self.weightAniScale/self.scaleStd**2 * (np.sum((rowScaleXZ - 1.)**2) + np.sum((rowScaleXZ[:2] - rowScaleXZ[2:])**2))
            errorAniScales = self.weightAniScale/self.scaleStd**2 * np.sum((rowScaleXZ - 1.)**2) 
    ...
    @staticmethod
    def constr1(x, limit=0.03):
        return limit - x[-4] + x[-2]
    @staticmethod
    def constr2(x, limit=0.03):
        return limit + x[-4] - x[-2]
    @staticmethod
    def constr3(x, limit=0.03):
        return limit - x[-3] + x[-1]
    @staticmethod
    def constr4(x, limit=0.03):
        return limit + x[-3] - x[-1]
    ...
    def maximization_stage1_step_5Views(self, rhobeg=1.0, maxiter=1000, verbose=True):
        ...
        limit = 0.02
        constraints = [{"type":"ineq","fun":constr_fun,"args":(limit,)} for constr_fun in [self.constr1,self.constr2,self.constr3,self.constr4]]
        resOptParams = scipy.optimize.minimize(fun=self.residualErrorOfGlobalParamsOf5Views, x0=x0, constraints=constraints,\
            args=(pIdx,False), method="COBYLA", options={"rhobeg":rhobeg,"maxiter":maxiter})
        params = resOptParams.x

        self.ex_rxyz, self.ex_txyz, self.focLth, self.dpix, self.u0, self.v0, self.rela_rxyz, self.rela_txyz =\
            self.parseGlobalParamsOf5Views(params, pIdx)
        self.rowScaleXZ = params[pIdx["rowScaleXZ"]:pIdx["rowScaleXZ"]+4]
    ...
    def anistropicRowScale2ScalesAndTransVecs(self):
        ...
        self.scales[:self.numUpperTooth] = np.prod(self.rowScaleXZ[:2])**(1/3) * np.ones_like(self.scales[:self.numUpperTooth], np.float32)
        self.scales[self.numUpperTooth:] = np.prod(self.rowScaleXZ[2:])**(1/3) * np.ones_like(self.scales[self.numUpperTooth:], np.float32)
        self.transVecXYZs[:self.numUpperTooth,[0,2]] = self.X_Mu_centroids[:self.numUpperTooth,[0,2]] * (self.rowScaleXZ[:2] - 1.)
        self.transVecXYZs[self.numUpperTooth:,[0,2]] = self.X_Mu_centroids[self.numUpperTooth:,[0,2]] * (self.rowScaleXZ[2:] - 1.)
        self.rowScaleXZ = np.ones((4,), np.float32)
    ...
    def maximization_stage2_step_5Views(self, rhobeg=0.1, maxiter=1000, step=1, verbose=True):
        x0, pIdx = self.getCurrentGlobalParamsOf5Views_as_x0()
        if step == 1:
            pIdx["tXYZs"] = len(x0)
    ...
    def residualErrorOfFeatureVec(self, featureVecParam, verbose):
        ...
        for phType in self.photoTypes:
            ...
            errorPixel = self.computePixelResidualError(phType, featureVec[tIdx], self.scales[tIdx], self.rotAngleXYZs[tIdx], self.transVecXYZs[tIdx], self.extrViewMat[ph], self.intrProjMat[ph],\
                self.rela_R, self.rela_txyz, rowScaleXZ=np.ones((4,),np.float32), stage=3)

...
class EMOpt5Views(object):
    ...
    def updateLowerPointPosByRelaPose(self, X_lower, rela_txyz):
        return X_lower + rela_txyz


    ###############################
    # world coord -> camera coord #
    ###############################
    def updateExtrinsicViewMatrix(self, ex_q, ex_txyz): # world coord to camera coord
    ...
    def updateEdgePrediction(self, photoType):
        ...
        if photoType in [PHOTO.LEFT, PHOTO.RIGHT, PHOTO.FRONTAL]:
            ...
            X_trans = np.concatenate([X_trans[:ul_sp], \
                self.updateLowerPointPosByRelaPose(X_trans[ul_sp:], self.rela_txyz)], axis=0) # left, right, frontal
    ...
    def computePixelResidualError(self, photoType, featureVec, scales, rotAngleXYZs, transVecXYZs, extrViewMat, intrProjMat,\
        rela_txyz, rowScaleXZ=np.ones((2,),np.float32), stage=1):
        ...
        if photoType in [PHOTO.LEFT, PHOTO.RIGHT, PHOTO.FRONTAL]:
            ul_sp = self.ul_sp[ph]
            X_trans_pred = X_trans_pred[:ul_sp] + [x+rela_txyz for x in X_trans_pred[ul_sp:]]
    ...
    def parseGlobalParamsOfSingleView(self, params, pIdx, photoType):
        ex_q = self.ex_q_lr * params[pIdx["ex_q"]:pIdx["ex_q"]+4]
        ...
        return ex_q, ex_txyz, focLth, dpix, u0, v0, rela_txyz

    def parseGlobalParamsOf5Views(self, params, pIdx):
        ex_q = self.ex_q_lr * params[pIdx["ex_q"]:pIdx["ex_q"]+20].reshape(5,4)
    ...
    def getGlobalParamsOfSingleView_as_x0(self, photoType, ex_q, ex_txyz, focLth, dpix, u0, v0, rela_txyz):
        # 生成 scipy.optimize.minimize 函数的部分输入
        ph = photoType.value
        pIdx = {"ex_q":0, "ex_txyz":4, "focLth":7, "dpix":8, "u0v0":9} # 与x0相对应
        x0 = np.hstack([ex_q[ph]/self.ex_q_lr, ex_txyz[ph]/self.ex_txyz_lr, \
            focLth[ph]/self.focLth_lr, dpix[ph]/self.dpix_lr, u0[ph]/self.uv_lr, v0[ph]/self.uv_lr])
        if photoType in [PHOTO.LEFT, PHOTO.RIGHT, PHOTO.FRONTAL]:
            pIdx.update({"rela_txyz":len(x0)})
            x0 = np.hstack([x0, rela_txyz/self.rela_txyz_lr])
        return x0, pIdx

    def getCurrentGlobalParamsOf5Views_as_x0(self):
        pIdx = {"ex_q":0, "ex_txyz":20, "focLth":35, "dpix":40, "u0":45, "v0":50,\
            "rela_txyz":55} # 与x0相对应
        x0 = np.hstack([self.ex_q.flatten()/self.ex_q_lr, self.ex_txyz.flatten()/self.ex_txyz_lr, \
    ...
    def residualErrorOfGlobalParams(self, params, pIdx, photoType, withAniScales=False, verbose=True):
        ...
        ph = photoType.value
        ex_q, ex_txyz, focLth, dpix, u0, v0, rela_txyz = self.parseGlobalParamsOfSingleView(params, pIdx, photoType)
        ...
        errorPixel = self.computePixelResidualError(photoType, self.featureVec[tIdx], self.scales[tIdx], self.rotAngleXYZs[tIdx], self.transVecXYZs[tIdx],\
            extrViewMat, intrProjMat, rela_txyz, rowScaleXZ, stage=1)
        if verbose == True:
    ...
    def maximization_stage1_step(self, photoType):
        # 优化 ex_q, ex_txyz, focLth, dpix, u0, v0, rela_txyz
        ph = photoType.value
        # prepare input x0
        x0, pIdx = self.getGlobalParamsOfSingleView_as_x0(photoType,\
            self.ex_q, self.ex_txyz, self.focLth, self.dpix, self.u0, self.v0, self.rela_txyz)
        # optimize
        ...
        # update
        self.ex_q[ph], self.ex_txyz[ph], self.focLth[ph], self.dpix[ph], self.u0[ph], self.v0[ph], self.rela_txyz =\
            self.parseGlobalParamsOfSingleView(params, pIdx, photoType)
        self.ex_q[ph] = self.ex_q[ph] / np.linalg.norm(self.ex_q[ph])


    def residualErrorOfGlobalParamsOf5Views(self, params, pIdx, verbose):
        # params = np.array([ex_ryz, ex_txyz, f, dpix, u0, v0, rowScaleXZ])
        ex_q, ex_txyz, focLth, dpix, u0, v0, rela_txyz = self.parseGlobalParamsOf5Views(params, pIdx)
        rowScaleXZ = params[pIdx["rowScaleXZ"]:pIdx["rowScaleXZ"]+2]
        errors = []
        for phType in self.photoTypes:
            sub_x0, sub_pIdx = self.getGlobalParamsOfSingleView_as_x0(phType, ex_q, ex_txyz, focLth, dpix, u0, v0, rela_txyz)
    ...
    def maximization_stage1_step_5Views(self, rhobeg=1.0, maxiter=1000, verbose=True):
        # 优化 ex_q, ex_txyz 与 focLth, dpix, u0, v0, 以及 rela_txyz, rowScaleXZ
        ...
        self.ex_q = self.ex_q / np.linalg.norm(self.ex_q, axis=1, keepdims=True)
    ...
    def residualErrorOfTeethPose(self, params, pIdx, photoType, verbose, step=1):
        ...
        errorPixel = self.computePixelResidualError(photoType, self.featureVec, scales, rotAngleXYZs, transVecXYZs, \
            extrViewMat, intrProjMat, rela_txyz, stage=2) # negative log likelihood of pixel distance distribution
    ...
    def residualErrorOfFeatureVec(self, featureVecParam, verbose):
        ...
        for phType in self.photoTypes:
            ...
            errorPixel = self.computePixelResidualError(phType, featureVec[tIdx], self.scales[tIdx], self.rotAngleXYZs[tIdx], self.transVecXYZs[tIdx], self.extrViewMat[ph], self.intrProjMat[ph],\
                self.rela_txyz, rowScaleXZ=np.ones((2,),np.float32), stage=3)

...
# '''Continue aligning correspondent point groups to minimize L2 error'''
# def continueAligningToothPointGroups(tempAlignedPointGroups, sRtParams):
#     pRef = np.array(tempAlignedPointGroups).mean(axis=0)
#     correPointGroups = [1/s * (pg-t) @ np.linalg.inv(r) for (s,r,t),pg in zip(sRtParams,tempAlignedPointGroups)]
#     centroids = [x.mean(axis=0) for x in correPointGroups]
#     meanCentroid = np.vstack(centroids).mean(axis=0)
#     scales = [param[0] for param in sRtParams]
#     rotMatrices = [param[1] for param in sRtParams]
#     meanRotAngleXYZ = getRotAngles(rotMatrices).mean(axis=0)
#     meanRotMat = getRotMat(meanRotAngleXYZ)
#     pRef = normalizePointArray(pRef, 1./np.array(scales).mean(), meanCentroid, meanRotMat.T)
...
if __name__ == "__main__": 
    ...
    # #  Run it only once  '''Continue aligning correspondent point groups to minimize L2 error'''
    ...
    # assert tags == trainPgTags
    # _params = [(s,R,t) for s,R,t in zip(scales, rotMats, tranVecs)]
    # _pgs = [np.loadtxt(os.path.join(SAVE_DIR,"{}.txt".format(_tag))) for _tag in tags]
    # alignedPointGroups, sRtParams = continueAligningToothPointGroups(_pgs, _params)

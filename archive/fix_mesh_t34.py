...
# 暫時不用
"""修补3,4号牙齿三角面片的大面积缺损"""
"""新增边为平均边长3倍时分割，feasible_thre = 0.7, 增加大三角网格上采样"""
...
def get_min_angle_index(v_bd, v_bd_ln, v_bd_rn, v_center):
    ...
    valid_v_bd_mask = bd_cv_norms > 5e-2
    ...
    feasible_thre = 0.7 # 这是一个经验值 cos值小于它被视为feasible
...
def getCoarsePatch(v_bd_id_init, v, f):
    ...
    while v_bd_id.size > 3:
        ...
        edgeRatio = np.floor(ln_rn_edge_l/(3*avg_edge_l)) # 3 测试
        if edgeRatio <= 1.0: # v_bd[min_angle_index] #边界上与左右顶点夹角最小的点
            ...
            for i in range(1, int(edgeRatio)):
                ...
                if i==1:
                    new_face = (v_bd_id[min_angle_index], v_bd_id[min_angle_index-1], new_v_id) # 注意三角面片中的点的方向保持一致
                    new_face_lst.append(new_face)
                else:
                    new_face = (v_bd_id[min_angle_index], new_v_id-1, new_v_id) # 注意三角面片中的点的方向保持一致
                    new_face_lst.append(new_face)
                if i==edgeRatio-1: # last face in this expansion
                    new_face = (v_bd_id[min_angle_index], new_v_id, v_bd_id[(min_angle_index+1)%v_bd_id.size]) # 注意三角面片中的点的方向保持一致
                    new_face_lst.append(new_face)
                new_v_id += 1
...
def generateOneRefinedPatchedTooth(srcDstObjFilePair, deletePrev=True):
    ...
    # 对三角网格中较大的三角形进行上采样
    finalV, finalF = trimesh.remesh.subdivide_to_size(vertices=v, faces=f, max_edge=2*avg_edge_length, max_iter=10, return_index=False)
    repairFaceWindingOrderAndNormals(finalV, finalF, dstObjFile)
...
if __name__ == "__main__":
    ...
    teethIndexRepairList = ["13","14","23","24","33","34","43","44"]
    teethIndexRepairList = []
    # teethIndexRepairList = ["15","16","17","25","26","27","35","36","37","45","46","47"]
    ...
    # # SINGLE TEST
    # srcDstPair = (os.path.join(srcObjRootDir,"44/1.obj"), os.path.join(dstObjRootDir,"44/1.obj"))

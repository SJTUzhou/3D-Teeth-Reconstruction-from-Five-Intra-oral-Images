...
class EMOpt5Views(object):
    ...
    def gridSearchRelativePoseParams(self):
        ...
        RelativePoseParamSearchSpace = {"rela.t.z": (np.array([-2,-1,0,1,2,6]), [PHOTO.LEFT, PHOTO.RIGHT, PHOTO.FRONTAL]),# tz:left,right,frontal
    ...
    def extractVisibleEdgePointsByAvgDepth(self, photoType):
        ...
        tooth_order = None
        X_uv_sort_groups = []
        if photoType in [PHOTO.LEFT, PHOTO.RIGHT]:
            ul_sp = self.ul_sp[ph]
            avg_depth_upper = avg_depth[:ul_sp]
            avg_depth_lower = avg_depth[ul_sp:]
            tooth_order_upper = avg_depth_upper.argsort()
            tooth_order_lower = ul_sp + avg_depth_lower.argsort()
            tooth_order = np.hstack([tooth_order_upper, tooth_order_lower])
            X_uv_sort_upper = self.X_uv[ph][tooth_order_upper]
            X_uv_sort_lower = self.X_uv[ph][tooth_order_lower]
            X_uv_sort_groups = [X_uv_sort_upper, X_uv_sort_lower]
        else: # PHOTO.UPPER, PHOTO.LOWER, PHOTO.FRONTAL
            tooth_order = avg_depth.argsort()
            X_uv_sort_ul = self.X_uv[ph][tooth_order]
            X_uv_sort_groups = [X_uv_sort_ul,]
        
        hulls = []
        vis_hull_vs = []
        vis_hull_vids = []
        for X_uv_sort in X_uv_sort_groups:
            _hulls = []
    ...
    def anistropicRowScale2ScalesAndTransVecs(self):
        ...
        # self.meanScales = np.prod(self.rowScaleXZ)**(1/3) * np.ones_like(self.scales, np.float64)
    ...
    def residualErrorOfGlobalParams(self, params, pIdx, photoType, withAniScales=False, verbose=True):
        ...
        rowScaleXZ = np.array([1.,1.], np.float64)
        errorAniScales = 0.
    ...
    def maximization_stage1_step(self, photoType, withAniScales=False):
        ...
        resOptParams = scipy.optimize.minimize(fun=self.residualErrorOfGlobalParams, x0=x0, \
            args=(pIdx, photoType, withAniScales, False), method="COBYLA", options={"rhobeg":1.0,"maxiter":1000}) # without rowScaleXZ
    ...
    def residualErrorOfFeatureVec(self, featureVecParam, verbose):
        ...
        for phType in self.photoTypes:
            ...
            errorPixel = self.computePixelResidualError(phType, featureVec[tIdx], self.scales[tIdx], self.rotAngleXYZs[tIdx], self.transVecXYZs[tIdx], self.extrViewMat[ph], self.intrProjMat[ph],\
                self.rela_R, self.rela_txyz, rowScaleXZ=np.ones((2,),np.float64), stage=3)

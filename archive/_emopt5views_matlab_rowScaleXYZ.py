...
class EMOpt5Views(object):
    ...
    def __init__(self, edgeMasks, photoTypes, visMasks, Mask, Mu, SqrtEigVals, Sigma, PoseCovMats, ScaleCovMat, transVecStd, rotAngleStd) -> None:
        ...
        self.rotAngleXYZs = np.zeros((self.numTooth,3), np.float64)
        self.transVecXYZs = np.zeros((self.numTooth,3), np.float64)
        
        self.rowScaleXYZ = np.ones((3,), dtype=np.float64) # 各向异性牙列放缩, 仅用于maximization stage1, 在stage2之前转化为scale和transVecXYZs
        ...
        self.weightViewMaxiStage1 = np.array([3.,3.,1.,1.,1.], dtype=np.float64)
        self.weightViewMaxiStage2 = np.array([3.,3.,1.,1.,1.], dtype=np.float64) 
    ...
    @staticmethod
    def chamferDistance(X1, X2):
        squared_D = distance_matrix(X1, X2, p=2, threshold=int(1e8))**2
        return np.mean(np.min(squared_D,axis=0)) + np.mean(np.min(squared_D,axis=1))
    ...
    def estimateCameraParams(self, phType):
        ...
        # 二维相似变换配准
        reg = cycpd.rigid_registration(**{'X': X, 'Y': Y, 'max_iterations':100,'tolerance':1.0,'verbose':False,'print_reg_params':False})
    ...
    def loss2D_chamferDistance(self, phType):
        ph = phType.value
        # rigid registration
        X = self.P_true[ph].astype(np.double)
        Y = self.P_pred[ph].astype(np.double)
        ...
        reg = cycpd.rigid_registration(**{'X': X, 'Y': Y, 'max_iterations':100,'tolerance':1.0,'verbose':False,'print_reg_params':False})
        TY,(s,r,t) = reg.register()
        return self.chamferDistance(X,TY)
        ...
        for phType, paramDict in ExtrParamSearchSpace.items():
            for paramName, paramSearchSpace in paramDict.items():
                ...
                for paramValue in paramSearchSpace:
                    ...
                    # loss = self.loss2D_chamferDistance(phType)
                    # print("2D chamfer distance loss: {:.4f}".format(loss))
                    # losses.append(loss)
    ...
    def gridSearchRelativePoseParams(self):
        ...
        RelativePoseParamSearchSpace = {"rela.t.z": (self.rela_txyz_default[2]+np.array([-2,-1,0,1,2]), [PHOTO.LEFT, PHOTO.RIGHT]),# tz:left,right
        ...
        for paramName, (paramSearchSpace, phTypes) in RelativePoseParamSearchSpace.items():
            ...
            for paramValue in paramSearchSpace:
                ...
                for phType in phTypes:
                    ...
                    # loss = loss + self.loss2D_chamferDistance(phType)
                # print("2D chamfer distance loss: {:.4f}".format(loss))
                losses.append(loss)
            bestParamValue = paramSearchSpace[np.argmin(losses)] # best guess from expectation loss
    ...
    def searchDefaultRelativePoseParams(self):
        ...
        bestParamValue = SearchSpace[np.argmin(losses)] # best guess from expectation loss
        self.assignValue2RelaPoseParamByName(paramName, bestParamValue, assign2DefaultValue=True) # update default values with the best guess
        print("Best param guess: ", bestParamValue)
        self.initExtrIntrParams(phType) # init extrinsic and intrinsic camera params with default values
        self.updateEdgePrediction(phType)
    ...
    def updateAlignedPointCloudInWorldCoord(self, tIdx):
        ...
        self.X_trans[tIdx] = self.rowScaleXYZ * self.X_trans[tIdx] # self.rowScaleXYZ = [1.,1.,1.] after maximization stage 1
    ...
    # def symmetric_expectation_loss(self, photoType, stage, verbose=True):
    ...
    #     loss = np.mean(np.min(loss_mat,axis=0)) + np.mean(np.min(loss_mat,axis=1))
    #     if verbose==True:
    #         print("symmetric expectation loss: {:.4f}".format(loss))
    #     return loss
    ...
    def save_expectation_step_result(self, filename):
        ...
        scipy.io.savemat(filename, {"np_invCovMatOfPose": self.invCovMats, "np_invCovMatOfScale": self.invCovMatOfScale,
                        ...
                        "np_u0": self.u0, "np_v0": self.v0, "np_rela_rxyz": self.rela_rxyz, "np_rela_txyz": self.rela_txyz, "np_rowScaleXYZ": self.rowScaleXYZ, 
                        ...
                        "np_visIdx": self.visIdx, "np_corre_pred_idx": self.corre_pred_idx, "np_P_true": self.P_true})
    ...
    def load_maximization_step_result(self, filename, stage):
        ...
        if stage == 0:
            self.rowScaleXYZ = np.array([1.,1.,1.],np.float64)
        elif stage == 1:
            self.rowScaleXYZ = xOptDict["rowScaleXYZ"]
        elif stage >= 2:
            self.scales = xOptDict["scales"]
            self.rotAngleXYZs = xOptDict["rotAngleXYZs"]
    ...
    def anistropicRowScale2ScalesAndTransVecs(self):
        ...
        # self.meanScales = np.prod(self.rowScaleXYZ)**(1/3) * np.ones_like(self.scales, np.float64)
        # self.meanTransVecXYZs[:,[0,2]] = self.X_Mu_centroids[:,[0,2]] * (self.rowScaleXYZ - 1.)
        self.scales = np.prod(self.rowScaleXYZ)**(1/3) * np.ones_like(self.scales, np.float64)
        self.transVecXYZs = self.X_Mu_centroids * (self.rowScaleXYZ - 1.)
        self.rowScaleXYZ = np.array([1.,1.,1.], np.float64)
    ...
    def computePixelResidualError(self, photoType, featureVec, scales, rotAngleXYZs, transVecXYZs, extrViewMat, intrProjMat,\
        rela_R, rela_txyz, rowScaleXYZ=np.ones((3,),np.float64), stage=1):
        ...
        if stage == 1: # 在优化相机参数同时，优化牙列的Anistropic scales
            X_corre_pred = rowScaleXYZ * X_corre_pred
    ...
    def residualErrorOfGlobalParams(self, params, pIdx, photoType, withAniScales=False, verbose=True):
        ...
        rowScaleXYZ = np.array([1.,1.,1.], np.float64)
        errorAniScales = 0.
        if withAniScales == True: # 考虑牙列各向异性放缩
            rowScaleXYZ = params[pIdx["rowScaleXYZ"]:pIdx["rowScaleXYZ"]+3] 
            errorAniScales = self.weightAniScale * np.sum((rowScaleXYZ - 1.)**2) / self.scaleStd**2
        ...
        errorPixel = self.computePixelResidualError(photoType, self.featureVec[tIdx], self.scales[tIdx], self.rotAngleXYZs[tIdx], self.transVecXYZs[tIdx],\
            extrViewMat, intrProjMat, rela_R, rela_txyz, rowScaleXYZ, stage=1)
        if verbose == True:
            print("errorPixel: {:.2f}, errorAniScales: {:.2f}".format(errorPixel, errorAniScales))
        return errorPixel + errorAniScales 
        
    
    def maximization_stage1_step(self, photoType, withAniScales=False):
        ...
        if withAniScales == True:
            pIdx["rowScaleXYZ"] = len(x0)
            x0 = np.hstack([x0, self.rowScaleXYZ])
        # optimize
        resOptParams = scipy.optimize.minimize(fun=self.residualErrorOfGlobalParams, x0=x0, \
            args=(pIdx, photoType, withAniScales, False), method="COBYLA", options={"rhobeg":1.0,"maxiter":1000}) # without rowScaleXYZ
        ...
        if withAniScales == True:
            self.rowScaleXYZ = params[pIdx["rowScaleXYZ"]:pIdx["rowScaleXYZ"]+3]


    def residualErrorOfGlobalParamsOf5Views(self, params, pIdx, verbose):
        # params = np.array([ex_ryz, ex_txyz, f, dpix, u0, v0, rowScaleXYZ])
        ...
        for phType in self.photoTypes:
            ...
            sub_pIdx["rowScaleXYZ"] = len(sub_x0)
            sub_x0 = np.hstack([sub_x0, rowScaleXYZ])
            ph = phType.value
    ...
    def maximization_stage1_step_5Views(self, rhobeg=1.0, maxiter=1000, verbose=True):
        # 优化 ex_rxyz, ex_txyz 与 focLth, dpix, u0, v0, 以及 rela_rxyz, rela_txyz, rowScaleXYZ
    ...
    def residualErrorOfFeatureVec(self, featureVecParam, verbose):
        ...
        for phType in self.photoTypes:
            ...
            errorPixel = self.computePixelResidualError(phType, featureVec[tIdx], self.scales[tIdx], self.rotAngleXYZs[tIdx], self.transVecXYZs[tIdx], self.extrViewMat[ph], self.intrProjMat[ph],\
                self.rela_R, self.rela_txyz, rowScaleXYZ=np.ones((3,),np.float64), stage=3)

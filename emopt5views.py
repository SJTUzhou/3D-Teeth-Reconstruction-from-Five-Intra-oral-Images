...
class EMOpt5Views(object):
    def __init__(self, edgeMasks, photoTypes, visMasks, Mask, Mu, Mu_normals, SqrtEigVals, Sigma, PoseCovMats, ScaleCovMat, transVecStd, rotVecStd) -> None:
        ...
        self.P_true = [np.argwhere(v>0)[:,::-1] for v in self.edgeMask] # xy position of detected boundary points in image coordinates
        self.P_true_normals = [self.initEdgeMaskNormals(v) for v in self.P_true] # normals at the detected boundary points in image coordinates
        self.M = [len(v) for v in self.P_true] # nums of detected boundary points in different photo
        ...
        self.ul_sp = {phType.value:np.argwhere(self.visIdx[phType.value] >= self.numUpperTooth).min() \
        ...
        self.rowScaleXZ = np.ones((2,), dtype=np.float32) # anistropic tooth row scale in xz plane, optimized in stage1, converted into tooth scales and transVecXYZs before stage 2
        ...
        self.rela_rxyz_default = np.array([0.,0.,0.],dtype=np.float32) 
        self.rela_txyz_default = np.array([0.,-5.,0.],dtype=np.float32) 
        self.rela_rxyz = None 
        self.rela_R = None
        self.rela_txyz = None 
        ...
        self.weight_point2point = 0.04  # param in residual pixel error in maximization loss
        self.weight_point2plane = 2.  # param in residual pixel error in maximization loss
        ...
        self.X_Mu_pred_normals = [None] * 5 # correponding 3D point normals of P_pred_normals in X_Mu_normals
    ...
    def initCameraIntrinsicParams(self, photoType):
        ph = photoType.value
        focLth = {PHOTO.UPPER:100.0, PHOTO.LOWER:100.0, PHOTO.LEFT:100.0, PHOTO.RIGHT:100.0, PHOTO.FRONTAL:100.0} 
        dpix = {PHOTO.UPPER:0.1, PHOTO.LOWER:0.1, PHOTO.LEFT:0.06, PHOTO.RIGHT:0.06, PHOTO.FRONTAL:0.06} 
    ...
    def gridSearchRelativePoseParams(self):
        ...
        RelativePoseParamSearchSpace = {"rela.t.z": (self.rela_txyz_default[2]+np.array([-3,-2,-1,0,1,2,3]), [PHOTO.LEFT, PHOTO.RIGHT]), 
                            "rela.t.y": (np.array([-7,-6,-5,-4]), [PHOTO.LEFT, PHOTO.RIGHT, PHOTO.FRONTAL]), 
                            "rela.t.x": (np.array([-1.,0.,1.]), [PHOTO.FRONTAL,])}  
    ...
    def computePixelResidualError(self, photoType, featureVec, scales, rotVecXYZs, transVecXYZs, extrViewMat, intrProjMat,\
        ...
        if stage >= 3: # consider deformation of tooth shape
            X_deformed_pred = [x_mu_pred + np.reshape(sqrtEigVal*fVec@sigmaTseg, x_mu_pred.shape) for x_mu_pred,sqrtEigVal,fVec,sigmaTseg in \
                              zip(self.X_Mu_pred[ph], self.SqrtEigVals[tIdx], featureVec, self.SigmaT_segs[ph])] 
            X_deformed_pred_normals = self.X_deformed_pred_normals[ph]
        
        if stage >= 2: # consider scales and poses of each tooth
            rotMats = self.computeRotMats(rotVecXYZs)
            _X_trans_pred = [s*np.matmul(x-tc,R)+t+tc for x,s,R,t,tc in zip(X_deformed_pred, scales, rotMats, transVecXYZs, self.X_Mu_centroids[tIdx])] 
        ...
        resPointError = self.weight_point2point * np.sum(np.linalg.norm(errorVecUV, axis=1)**2) 
        resPlaneError = self.weight_point2plane * np.sum(np.sum(errorVecUV*P_corre_pred_normals, axis=1)**2)
        ...
        par_loss_par_hatci = -2./ _M * np.matmul(ci_hatci[:,None,:], \
            (self.weight_point2point * np.identity(2,np.float32) + self.weight_point2plane * np.matmul(hatni[:,:,None],hatni[:,None,:]))) #(_M, 1, 2)
        par_loss_par_hatni = 2*self.weight_point2plane/_M * ci_hatci_dot_hatni[:,None,None] * ci_hatci.reshape(_M,1,2)  #(_M, 1, 2)
        ...
        if stage == 1: # gradient  w.r.t anistropic scale of tooth row in stage 1
            ...
            numT = self.numTooth
            tIdx = self.visIdx[ph]
            qt_list = [x-tc for x,tc in zip(_X_trans_pred,self.X_Mu_centroids[tIdx])] 
